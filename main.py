...
class BottleCheckerApp:
    ...
    def publish_result(self, status, detail="", trigger=""):
        try:
            if status == "OK":
                msg = "OK"
            ...
            # Option : message plus riche
            # msg = f"{msg}|{detail[:40]}|{trigger[:20]}"
            ...
            self.log(f"→ Publié {self.MQTT_TOPIC_RESPONSE} : {msg}")
    ...
    def perform_analysis(self, auto, trigger_payload):
        try:
            ...
            # ─── ENVOI MQTT RÉPONSE ───────────────────────────────
            self.publish_result(etat, detail, trigger_payload)

        except Exception as e:
            self.log(f"Erreur pendant l'analyse : {e}")
            self.publish_result("ERROR", str(e), trigger_payload)
        finally:
            self.analyzing = False
